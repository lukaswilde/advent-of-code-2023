...
fn main() {
    ...
    if let Some(cal_numbers) = calibration_numbers {
        ...
        println!("The sum of original calibration values is {}", calibration_sum);
    } else {
    ...
    if let Some(mod_cal_numbers) = mod_calibration_numbers {
        ...
        println!("The sum of transformed calibration values is {}", calibration_sum);
    } else {
...
fn transform_numbers(lines: &Vec<String>) -> Vec<String> {
    lines
        ...
        .map(|line| {
            ...
            res.into_iter().filter_map(|i| char::from_digit(i, 10)).collect()
        })
        .collect()
}
...
fn get_calibration_number(line: &String) -> Option<u32> {
    ...
    if filtered.is_empty() {
        ...
        Some(10 * filtered.first().expect("First digit must exist")
